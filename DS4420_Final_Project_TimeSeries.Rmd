...
Try AR(12) since there is the decomposition for adoptions showed a yearly seasonal pattern too.

```{r}
# isolate necessary columns for AR(12)
adoptions_ar12 <- month_data_lags[, c("MonthYear", "Adoption", paste0("Lag_", 1:12))]

# split train (up to May 2024) and test (May 2024 to May 2025) sets
train_data <- subset(adoptions_ar12, MonthYear <= as.Date("2024-05-01"))
test_data  <- subset(adoptions_ar12, MonthYear > as.Date("2024-05-01"))

# only non-null lag
train_data <- train_data[complete.cases(train_data[, c("Adoption", paste0("Lag_", 1:12))]), ]
...
# AR(12) model input
X_train <- train_data[, paste0("Lag_", 1:12)]
```

```{r}
# fit AR(12) Model with intercept (OLS)
X_matrix <- as.matrix(cbind(Intercept = 1, X_train))
y_vector <- matrix(y_train, ncol = 1)
w <- solve(t(X_matrix) %*% X_matrix) %*% (t(X_matrix) %*% y_vector)
w
```
...
start <- as.numeric(tail(X_train, 1))

for (i in 1:n_test) {
  y_pred[i] <- w[1] + sum(w[-1] * start)
  start <- c(y_pred[i], start[1:11])
}
```

```{r}
# observed vs predicted plot
plot(y_pred, y_test, xlab = "Predicted Adoption", ylab = "Observed Adoption", main = "AR(12) Model Predictions for Animal Adoptions")
abline(0, 1, col = "red")
```
...
plot(test_data$MonthYear, y_test, type = "l", col = "blue", lwd = 2, 
     ylim = range(c(y_pred, y_test)), xlab = "Month", ylab = "Adoption Count", 
     main = "AR(12) Model Predictions for Animal Adoptions")
lines(test_data$MonthYear, y_pred, col = "red", lwd = 2, lty = 2)
legend("bottomleft", legend = c("Actual", "AR(12) Predicted"), col = c("blue", "red"), lty = c(1, 2), bty = "n")
```

```{r}
# check average monthly adoptions for comparison
mean(adoptions_ar12$Adoption)
...
# create full MonthYear sequence
all_months <- seq.Date(from = min(adoptions_ar12$MonthYear),
                       by = "month",
                       length.out = nrow(adoptions_ar12) + 24)

# where prediction starts (May 2024+)
pred_start_index <- which(all_months == min(test_data$MonthYear))

# plot full time series
plot(adoptions_ar12$MonthYear, adoptions_ar12$Adoption, type = "l", col = "blue", lwd = 2,
  xlim = range(all_months), ylim = range(c(adoptions_ar12$Adoption, y_pred)), xlab = "Month",
  ylab = "Adoption Count", main = "AR(12) Model Predictions and Long-Term Forecast for Adoptions")
lines(all_months[pred_start_index : (pred_start_index + n_future - 1)], y_pred, col = "red", lwd = 2, lty = 2)
legend("bottomleft", legend = c("Actual", "AR(12) Predicted"), col = c("blue", "red"), lty = c(1, 2), bty = "n")
```
...
Try AR(12) since there is the decomposition for transfers showed a yearly seasonal pattern too.

```{r}
# isolate necessary columns for AR(12)
transfers_ar12 <- month_data_lags[, c("MonthYear", "Transfer", paste0("LagT_", 1:12))]

# split train (up to May 2024) and test (May 2024 to May 2025) sets
train_data <- subset(transfers_ar12, MonthYear <= as.Date("2024-05-01"))
test_data  <- subset(transfers_ar12, MonthYear > as.Date("2024-05-01"))

# only non-null lag
train_data <- train_data[complete.cases(train_data[, c("Transfer", paste0("LagT_", 1:12))]), ]
...
# AR(12) model input
X_train <- train_data[, paste0("LagT_", 1:12)]
```
...
```{r}
# observed vs predicted plot
plot(y_pred, y_test, xlab = "Predicted Transfer", ylab = "Observed Transfer", main = "AR(12) Model Predictions for Animal Transfers")
abline(0, 1, col = "red")
```
...
plot(test_data$MonthYear, y_test, type = "l", col = "blue", lwd = 2, 
     ylim = range(c(y_pred, y_test)), xlab = "Month", ylab = "Transfer Count", 
     main = "AR(12) Model Predictions for Animal Transfers")
lines(test_data$MonthYear, y_pred, col = "red", lwd = 2, lty = 2)
legend("bottomleft", legend = c("Actual", "AR(12) Predicted"), col = c("blue", "red"), lty = c(1, 2), bty = "n")
```
...
# create full MonthYear sequence
all_months <- seq.Date(from = min(transfers_ar12$MonthYear),
                       by = "month",
                       length.out = nrow(transfers_ar12) + 24)

# where prediction starts (May 2024+)
pred_start_index <- which(all_months == min(test_data$MonthYear))

# plot full time series
plot(transfers_ar12$MonthYear, transfers_ar12$Transfer, type = "l", col = "blue", lwd = 2,
  xlim = range(all_months), ylim = range(c(transfers_ar12$Transfer, y_pred)), xlab = "Month",
  ylab = "Transfer Count", main = "AR(12) Model Predictions and Long-Term Forecast for Transfers")
lines(all_months[pred_start_index : (pred_start_index + n_future - 1)], y_pred, col = "red", lwd = 2, lty = 2)
legend("bottomleft", legend = c("Actual", "AR(12) Predicted"), col = c("blue", "red"), lty = c(1, 2), bty = "n")
```
